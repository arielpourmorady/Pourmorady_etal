...
z <- ggplot(big_data, aes(x = compartment, y = size)) +
  geom_violin(fill = "grey") + 
  ggtitle("Library Normalized") + 
...
# Square Normalized per Cell Contacts #


square_norm_percell <- function(specific_datalist, compartment_name){
  dataframe = data.frame()
  for (i in specific_datalist){
    if (is.null(i) != TRUE) {
      df <- i %>% mutate(size = size/sum(size)) %>%
        filter(bait_order == 20, prey_order == 20) %>%
...
ORs_in_GIH_datalist_active_P2_update <-square_norm_percell(ORs_in_GIH_datalist_active_P2, "Active OR Comp")
ORs_in_GIH_datalist_active_MOR28_update <-square_norm_percell(ORs_in_GIH_datalist_active_MOR28, "Active OR Comp")
ORs_in_GIH_datalist_inactive_P2_update <-square_norm_percell(ORs_in_GIH_datalist_inactive_P2, "Inactive OR Comp")
ORs_in_GIH_datalist_inactive_MOR28_update <-square_norm_percell(ORs_in_GIH_datalist_inactive_MOR28, "Inactive OR Comp")

active_islands_datalist_P2_update <-square_norm_percell(active_islands_datalist_P2, "Active GIH")
active_islands_datalist_MOR28_update <-square_norm_percell(active_islands_datalist_MOR28, "Active GIH")
inactive_islands_datalist_P2_update <-square_norm_percell(inactive_islands_datalist_P2, "Inactive GIH")
inactive_islands_datalist_MOR28_update <-square_norm_percell(inactive_islands_datalist_MOR28, "Inactive GIH")

big_data <- rbind(ORs_in_GIH_datalist_active_P2_update, ORs_in_GIH_datalist_active_MOR28_update,
                  ORs_in_GIH_datalist_inactive_P2_update, ORs_in_GIH_datalist_inactive_MOR28_update,
                  active_islands_datalist_P2_update,active_islands_datalist_MOR28_update,
                  inactive_islands_datalist_P2_update, inactive_islands_datalist_MOR28_update)

big_data$compartment <- factor(big_data$compartment, levels = unique(big_data$compartment))

my_comparisons <- list( c("Active OR Comp", "Inactive OR Comp"), c("Active GIH", "Inactive GIH"))

x <- ggplot(big_data, aes(x = compartment, y = size)) +
  geom_violin(fill = "grey") + 
  ggtitle("Square Normalized per Cell") + 
  stat_compare_means(method = "t.test", comparisons = my_comparisons) + 
  theme_classic()
