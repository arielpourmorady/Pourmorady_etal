...
a <- ggplot(scatter.5min %>% filter(bait_loc > 200000000, bait_loc < 500000000), aes(x = bait_loc, y = LOS, group = 1)) + 
  geom_line(colour = "blue") +
  geom_line(data = scatter.30min %>% filter( bait_loc > 200000000, bait_loc < 500000000), aes(x = bait_loc, y = LOS, group = 1), colour = "green4") + 
  geom_line(data = scatter.60min %>% filter( bait_loc > 200000000, bait_loc < 500000000), aes(x = bait_loc, y = LOS, group = 1), colour = "darkorchid4") + 
  theme_classic() +
  xlab("Chr 2 Position") +
  scale_x_discrete(breaks = scatter.5min$bait_loc[seq(1, length(scatter.5min$bait_loc), by = 100)])


b <- ggplot(scatter.5min %>% filter(bait_loc > 200000000, bait_loc < 500000000), aes(x = bait_loc, y = eigen, group = 1)) + 
  geom_line() +
  theme_classic()+
  xlab("Chr 2 Position") +
  scale_x_discrete(breaks = scatter.5min$bait_loc[seq(1, length(scatter.5min$bait_loc), by = 100)]) +
  ylab("A/B Compartment")


df <- left_join(scatter.5min, gg8.p2.norm, by = "bait")

d <- ggplot(df %>% filter(bait_loc > 200000000, bait_loc < 500000000), aes(x = bait_loc, y = foldchange, group = 1)) + 
  geom_line(colour = "firebrick3") +
  theme_classic()+
  xlab("Chr 2 Position") +
  scale_x_discrete(breaks = df$bait_loc[seq(1, length(df$bait_loc), by = 100)]) +
  ylab("H3K27ac HiChIP")

c <- ggarrange(d, a, b, nrow = 3, align = "v") # LOS vs. Eigen score
c
...
scatter.filtered <- scatter %>% filter(eigen < 1) %>%
  group_by(OR, type) %>% summarise(median = mean(LOS))
geno.levels <- c("5-min", "30-min", "60-min")
scatter.filtered$type <- factor(scatter.filtered$type,levels=geno.levels, ordered=TRUE)

ggplot(scatter.filtered, aes(x = type, y = median, colour = OR)) + geom_point(size = 3) + 
  xlab("Liquification Time") + 
  ylab("Loss of Structure") + 
  ggtitle("Chr 4")
