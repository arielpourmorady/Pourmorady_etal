...
setwd("/media/storageE/ariel/R/finalpaper_August2023/dipc/dipc.ptIII.GIHDeterminism") 
...
distance_from_active_allele_dataframe_P2_filter <- distance_from_active_allele_dataframe_P2 %>%
  ...
  separate(island, sep =',', into = c('chr', 'loc', 'name', NA)) %>%
  mutate(island = paste(chr, loc, name, sep = ',')) %>%
  filter(!grepl('chr7', island)) %>%
  filter(!grepl('chr14', island)) %>%
  group_by(OR, cell, island) %>%
  summarise(count = n())

distance_from_active_allele_dataframe_P2_filter$cell %>% unique() %>% length()
...
P2_df_clean <- P2_df_clean[!duplicated(data.frame(t(apply(P2_df_clean[1:2], 1, sort)), P2_df_clean$a)),]
nrow(P2_df_clean)
...
distance_from_active_allele_dataframe_MOR28_filter$cell %>% unique() %>% length()
...
MOR28_df_clean <- MOR28_df_clean[!duplicated(data.frame(t(apply(MOR28_df_clean[1:2], 1, sort)), MOR28_df_clean$a)),]
nrow(MOR28_df_clean)
...
combined_df_clean <- combined_df_clean[!duplicated(data.frame(t(apply(combined_df_clean[1:2], 1, sort)), combined_df_clean$a)),]
nrow(combined_df_clean)
...
write.table(final_data, file = "/media/storageE/ariel/R/finalpaper_August2023/dipc/dipc.ptIII.GIHDeterminism/SuppFig5c_GIDeterminism.txt")
