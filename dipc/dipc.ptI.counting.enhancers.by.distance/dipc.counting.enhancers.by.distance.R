...
working_directory <- "/media/storageE/ariel/R/finalpaper_August2023/dipc/dipc.ptI.counting.enhancers.by.distance/"
...
names_col <- read.table("OR.no_GI_overlap_50kb.nonX.names") %>%
  separate(V1, into = c("chr", "loc", "olfr", "allele"), sep = ",") %>%
  mutate(name = paste(olfr, allele, sep = "_"))
names_row <- read.table("Greek_Islands.nonX.names")


##############################################################################
################ FUNCTIONS FOR ANALYSIS AND DATA EXTRACTION ##################
##############################################################################
...
enhancer_heatmap <- function(enhancers_distance, olfr, chr) {
  df <- enhancers_distance %>% 
    ...
    mutate(count = count/(pi*2*distance)) %>%
...
male_P2_files <- list.files(path = "matrix.P2-ImmFix-dipc.male.Greek_Islands.nonX.L1.OR.no_GI_overlap_50kb.nonX.L2.pd",
...
female_P2_files <- list.files(path = "matrix.P2-ImmFix-dipc.female.Greek_Islands.nonX.L1.OR.no_GI_overlap_50kb.nonX.L2.pd",
                              "*.20k.1.clean.Greek_Islands.nonX.L1.OR.no_GI_overlap_50kb.nonX.L2.pd.txt",
                              full.names = TRUE,
                              recursive = FALSE)
...
male_mor28_files <- list.files(path = "matrix.APM28DIP.male.Greek_Islands.nonX.L1.OR.no_GI_overlap_50kb.nonX.L2.pd",
                               "*.20k.1.clean.Greek_Islands.nonX.L1.OR.no_GI_overlap_50kb.nonX.L2.pd.txt",
                               full.names = TRUE,
                               recursive = FALSE)
...
female_mor28_files <- list.files(path = "matrix.APM28DIP.female.Greek_Islands.nonX.L1.OR.no_GI_overlap_50kb.nonX.L2.pd",
                                 "*.20k.1.clean.Greek_Islands.nonX.L1.OR.no_GI_overlap_50kb.nonX.L2.pd.txt",
                                 full.names = TRUE,
                                 recursive = FALSE)
...
# analyzing p2 in the p2 dataset
...
### Export
preparingData <- function(dataframe, olfr, allele, genotype){
  df <- dataframe %>%
    dplyr::rename("giDistance" = "Var1",
                  "meanNormalizedGIs" = "mean") %>%
    mutate(OR = olfr, parent = allele, dataset = genotype)
  return(df)
}

p2_bl6_dist <- preparingData(p2_bl6_dist, "P2", "nonCas", "P2 DipC")
p2_CAST_dist <- preparingData(p2_CAST_dist, "P2", "Cas", "P2 DipC")
mor28_bl6_dist <- preparingData(mor28_bl6_dist, "mor28", "nonCas", "P2 DipC")
mor28_CAST_dist <- preparingData(mor28_CAST_dist, "mor28", "Cas", "P2 DipC")

P2_export <- rbind(p2_bl6_dist, p2_CAST_dist, mor28_bl6_dist, mor28_CAST_dist)

write.table(P2_export, file = "/media/storageE/ariel/R/finalpaper_August2023/dipc/dipc.ptI.counting.enhancers.by.distance/SuppFig4c.txt",
            sep = '\t', col.names = TRUE)
...
p2_bl6_dist <- preparingData(p2_bl6_dist, "P2", "nonCas", "mor28 DipC")
p2_CAST_dist <- preparingData(p2_CAST_dist, "P2", "Cas", "mor28 DipC")
mor28_bl6_dist <- preparingData(mor28_bl6_dist, "mor28", "nonCas", "mor28 DipC")
mor28_CAST_dist <- preparingData(mor28_CAST_dist, "mor28", "Cas", "mor28 DipC")

mor28_export <- rbind(p2_bl6_dist, p2_CAST_dist, mor28_bl6_dist, mor28_CAST_dist)

write.table(mor28_export, file = "/media/storageE/ariel/R/finalpaper_August2023/dipc/dipc.ptI.counting.enhancers.by.distance/SuppFig4d.txt",
            sep = '\t', col.names = TRUE)
