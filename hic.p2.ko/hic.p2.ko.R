...
cis_hic_path <- c(omp.gfp.cis_hic_path,
                  ...
                  atf5.cis_hic_path,
                  ngn.cis_hic_path,
                  hbc.cis_hic_path)#Here are the cis contacts  , P2.cis_hic_path, OMP.cis_hic_path, , control.acid.r1.cis_hic_path, liquid.HiC.control.cis_hic_path,liquid.HiC.30.cis_hic_path,liquid.HiC.60.cis_hic_path
...
hic_contacts %>% select(geno) %>% unique()
...
grid.arrange(a[[4]], b[[4]], c[[4]], d[[4]], e[[4]], f[[4]], g[[4]])
...
# Stavros Requests
enhancer_to_enhancer_matrix.omp.gfp <- enhancer_to_enhancer_matrix(hic_contacts, "omp.gfp", "trans", "none")
...
enhancer_to_enhancer_matrix.omp.m71.ko <- enhancer_to_enhancer_matrix(hic_contacts, "omp.m71.ko", "trans", "none")
enhancer_to_enhancer_matrix.omp.m71.ko %>% filter(bait_order == 20, prey_order == 20)
enhancer_to_enhancer_matrix.omp.m71.ko <- enhancer_to_enhancer_matrix.omp.m71.ko %>% dcast(bait_order ~ prey_order, value.var = "norm")

enhancer_to_enhancer_matrix.gg8.p2.ko <- enhancer_to_enhancer_matrix(hic_contacts, "gg8.p2.ko", "trans", "none")
enhancer_to_enhancer_matrix.gg8.p2.ko %>% filter(bait_order == 20, prey_order == 20)
enhancer_to_enhancer_matrix.gg8.p2.ko <- enhancer_to_enhancer_matrix.gg8.p2.ko %>% dcast(bait_order ~ prey_order, value.var = "norm")

enhancer_to_enhancer_matrix.atf5 <- enhancer_to_enhancer_matrix(hic_contacts, "atf5", "trans", "none")
...
i <- pheatmap(enhancer_to_enhancer_matrix.omp.m71.ko[,-1], 
              cluster_rows=FALSE, 
              cluster_cols=FALSE, 
              border_color = NA,
              show_rownames = FALSE,
...
k <- pheatmap(enhancer_to_enhancer_matrix.gg8.p2.ko[,-1], 
              cluster_rows=FALSE, 
              cluster_cols=FALSE, 
              border_color = NA,
              show_rownames = FALSE,
...
a <- ggplot(df, aes(x = prey_order, y = norm, colour = geno)) + 
  geom_line(size = 0.75) + 
...
###########################################
#### Cummulative GI Contacts to all ORs ###
###########################################
...
oneOR_allGIs_df_omp.gfp <- oneOR_allGIs_df %>% dplyr::filter(geno == "omp.gfp")
oneOR_allGIs_df_omp.m71.ko <- oneOR_allGIs_df %>% dplyr::filter(geno == "omp.m71.ko") %>% 
  left_join(oneOR_allGIs_df_omp.gfp, by = "prey") %>%
  mutate(FC = sum_norm.x/sum_norm.y)
oneOR_allGIs_df_gg8.p2 <- oneOR_allGIs_df %>% dplyr::filter(geno == "gg8.p2")%>% 
  left_join(oneOR_allGIs_df_omp.gfp, by = "prey") %>%
  mutate(FC = sum_norm.x/sum_norm.y)

a <- ggplot(oneOR_allGIs_df_gg8.p2, aes(x = prey, y = FC)) + geom_point() + ylim(0, max(oneOR_allGIs_df_gg8.p2$FC)) 
b <- ggplot(oneOR_allGIs_df_omp.m71.ko, aes(x = prey, y = FC)) + geom_point() + ylim(0, max(oneOR_allGIs_df_gg8.p2$FC))
...
paletteLength <- 100
myColor <- colorRampPalette(c("white", "firebrick3"))(paletteLength)
myBreaks <- c(seq(min(activeP2_to_enhancer_matrix.atf5[,-1]), 
                  max(activeP2_to_enhancer_matrix.atf5[,-1]), 
                  length.out=ceiling(paletteLength)))
...
b <- pheatmap(activeP2_to_enhancer_matrix.atf5[,-1], 
              cluster_rows=FALSE, 
              cluster_cols=FALSE, 
              border_color = NA,
              show_rownames = FALSE,
...
oneGI_allGIs_df <- hic_contacts %>%
  dplyr::filter(bait %in% Islands_50kb$prey) %>%
  dplyr::filter(prey %in% Islands_50kb$prey) %>% 
  dplyr::filter(arch == "trans") %>%
  dplyr::filter(geno %in% c("hbc", "ngn", "atf5", "omp.gfp")) %>%
  group_by(prey, geno, arch) %>%
  summarise(sum_norm = sum(norm))

oneGI_allGIs_df$geno <- factor(oneGI_allGIs_df$geno, levels = c("hbc", "ngn", "atf5", "omp.gfp"))
...
ggplot(P2_allGIs_df, aes(x = geno, y = sum_norm, fill = arch)) + 
