...
# P2 Analysis
islands_percell_dataframe_P2_mean <- read.table("/media/storageE/ariel/R/finalpaper/dipc.inactive.hub.extraction/islands_percell_dataframe_P2.txt",sep = "\t") %>%
...
islands_percell_dataframe_MOR28_mean <- read.table("/media/storageE/ariel/R/finalpaper/dipc.inactive.hub.extraction/islands_percell_dataframe_MOR28.txt",sep = "\t") %>%
  separate(col = "element", sep = ",", into = c("chr", "loc", "island", "haplo")) %>%
  mutate("name" = paste(island, haplo, sep = ",")) %>%
  dplyr::select(name, state, cell) %>%
  dplyr::filter(state == "active") %>%
...
islands_percell_dataframe_MOR28_mean <- read.table("/media/storageE/ariel/R/finalpaper/dipc.inactive.hub.extraction/islands_percell_dataframe_MOR28.txt",sep = "\t") %>%
  ...
  summarise(mean = mean(sum),
            ...
            upper = mean + sd) %>%
  dplyr::filter(state == "active")
islands_percell_dataframe_MOR28_mean
...
male_files <- list.files(path = "/media/storageE/ariel_dipc/P2-ImmFix-dipc/matrix.P2-ImmFix-dipc.male.Greek_Islands.nonX.L1.Greek_Islands.nonX.L2.pd",
                         "*.20k.1.clean.Greek_Islands.nonX.L1.Greek_Islands.nonX.L2.pd.txt",
                         full.names = TRUE,
                         recursive = FALSE)
female_files <- list.files(path = "/media/storageE/ariel_dipc/P2-ImmFix-dipc/matrix.P2-ImmFix-dipc.female.Greek_Islands.nonX.L1.Greek_Islands.nonX.L2.pd",
                           "*.20k.1.clean.Greek_Islands.nonX.L1.Greek_Islands.nonX.L2.pd.txt",
                           full.names = TRUE,
                           recursive = FALSE)
...
female_files <- list.files(path = "/media/storageE/AP_LB_dipc/matrix.mor28.Greek_Islands.nonX.L1.Greek_Islands.nonX.L2.pd",
                           "APM28DIP.*.female.20k.1.clean.Greek_Islands.nonX.L1.Greek_Islands.nonX.L2.pd.txt",
                           full.names = TRUE,
                           recursive = FALSE)

male_files <- list.files(path = "/media/storageE/AP_LB_dipc/matrix.mor28.Greek_Islands.nonX.L1.Greek_Islands.nonX.L2.pd",
