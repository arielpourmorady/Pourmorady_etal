...
setwd("/media/storageE/ariel/R/finalpaper_August2023/rna/rna.OMPM71nc_v_OMPGFPOMPtTA/") 
...
dds$condition <- factor(dds$condition, levels=c('OMP_GFP_OMPtTA',
                                                'OMPtTA_tetOM71_KO'))
...
resLFC <- lfcShrink(dds, contrast = c("condition", "OMPtTA_tetOM71_KO", "OMP_GFP_OMPtTA"), type="normal")
...
ggplot(resLFC_d %>% arrange(sig), aes(x = baseMean, y = log2FoldChange)) + 
  ...
  ggtitle("OMPtTA_tetOM71_KO vs. OMP_GFP_OMPtTA") + 
...
resLFC_d$comparison <- "OMPtTA_tetOM71_KO vs. OMP_GFP_OMPtTA"
write.table(resLFC_d, file = "SuppFig10i_MAPlot.txt")
